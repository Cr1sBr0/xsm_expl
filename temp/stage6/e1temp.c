...
void typecheck(struct Typetable* t1, struct Typetable* t2, char c) {
    ...
    switch(c) {
        ...
        case 'a': if(t1 !=TLookup("int") || t2!=TLookup("int")) {   //arithmetic operation
                  ...
                  break;
        case 'r': if(t1 !=TLookup("int")  || t2!=TLookup("int") ) { //comparison operation
...
struct tNode* getFieldNode(char *name,char *name2){
	struct tNode* a=(struct tNode*)malloc(sizeof(struct tNode));
	a->nodetype=13;
	a->varname=(char*)malloc(sizeof(name));
	strcpy(a->varname,name);
	return addMember(a,name2);
}
struct tNode*addMember(struct tNode* node,char *name){
	struct tNode* a=(struct tNode*)malloc(sizeof(struct tNode));
	a->nodetype=14;
	a->varname=(char*)malloc(sizeof(name));

	strcpy(a->varname,name);
	struct argList* c=(struct argList*)malloc(sizeof(struct argList));
	c->node=a;
	c->next=NULL;
	struct argList*b=node->argList;
	if(b==NULL){
		node->argList=c;
		return node;
	}
